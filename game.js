...
function sortStructuresForView(list) {
  ...
  arr.sort((a, b) => {
    ...
    if (mode === 'borgoAsc')   return getBorgoName(a).localeCompare(getBorgoName(b), 'it'); 
    if (mode === 'borgoDesc')  return getBorgoName(b).localeCompare(getBorgoName(a), 'it'); 
...
state.lastAnnualReport = {
  ...
  // snapshot prestiti + budget finali

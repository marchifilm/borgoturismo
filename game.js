...
function chooseAccessIdentity(){
  ...
  state.access.playerId = null; // verrà agganciato dopo lo snapshot
...
const loanRows = (p.loans && p.loans.length)
  ...
  card.innerHTML = `
    ...
    <div>Prestiti attivi: <strong>€ ${fmt(loans)}</strong> (10% annuo)</div>
...
if(window.saveToFirebase) saveToFirebase(state);
  ...
  document.getElementById('applyMarketing').onclick=()=>{
    ...
    ensureYearBook(p.id); state.annualBook[p.id].marketing += p.marketing;
...
if(window.saveToFirebase) saveToFirebase(state);
  ...
  document.getElementById('takeLoanBtn').onclick=()=>{
    ...
    if(!p.loans) p.loans=[]; p.loans.push({principal:val,rate:0.10,year:state.year}); p.budget+=val;
    log(`${p.name} prende prestito €${fmt(val)} (10% annuo)`); renderPlayers(); renderPlayerCard();
if(window.saveToFirebase) saveToFirebase(state);
  };
...
<td>
  <input class="cell" type="number"
    min="1"
    max="${s.qualita <= 3 ? 5 : s.qualita === 4 ? 6 : 7}"
    value="${s.camere}"
...
const maxCamere = qualita <= 3 ? 5 : qualita === 4 ? 6 : 7;
if (camere > maxCamere) {
...
function premiumMarketFill(seas){
  const base = 0.70;                  // era 0.55 (troppo basso)
  const seasonal = 0.45 * seas.demand; // era 0.35
  return clamp01(base + seasonal);     // arriva spesso vicino a 1 in alta stagione
}

function premiumChoiceShare(structs, s, state){
  const BETA_PRICE = 0.0022;
  const BETA_QUAL  = 0.28;
  const BETA_POOL  = 0.18;
  const BETA_BORGO = 0.22;
  const BETA_CLEAN = 0.12;
  const BETA_REVIEW= 0.10;

  const OUTSIDE_U = -0.30;
...
document.getElementById('btnShowAnnual').addEventListener('click', ()=> openAnnualReport(false));
...
function monthlyShock(){
  if(Math.random()*100 < state.params.eventProbMonth){
    ...
    const mag = 0.2; // ±20%
    return { active:true, target:t, factor:1+sign*mag, desc:`${t} ${sign<0?'-':'+'}20%` };
  }
...
function monthlyMgmtCost(s){
  ...
  // incremento per camere oltre la quinta
  if (s.camere > 5) {
    const extraRooms = s.camere - 5;
    const scaleFactor = 1 + 0.20 * extraRooms; // +20% per ogni camera in più
    annual *= scaleFactor;
  }
...
function dynamicTargetPrice(s, state, seas, P){
  // 1) ancora “macro” al potere d’acquisto del mese
  // avgSpend è il centro della domanda del mercato; seas.priceAmp fa salire/scendere la tolleranza al prezzo
  const base = (P.avgSpend || 120) * (0.92 + 0.10 * (seas.demand - 1)); 
  // (in alta stagione base sale un po’, in bassa scende un po’)

  // 2) qualità: premium cresce in modo più che lineare dal 3 in su (per non far vincere sempre Q3)
  const q = s.qualita || 3;
  const qMul =
    q === 1 ? 0.55 :
    q === 2 ? 0.80 :
  ...
  // 3) borgo: più forte per premium/luxury
  const borgoW = (state.borgoValues?.[s.borgo] || 1);
  const borgoMul = Math.pow(borgoW, q >= 3 ? 0.45 : 0.25);

  // 4) servizi: piscina “sposta” in alto il target (puoi chiedere di più)
  const poolMul = s.piscina ? 1.10 : 1.00;

  // 5) concorrenza: se nello stesso “segmento” ci sono molti concorrenti, il target scende un filo
  const segMinQ = (q >= 4) ? 4 : (q === 3 ? 3 : 1);
  const segMaxQ = (q >= 4) ? 5 : (q === 3 ? 3 : 2);
  const competitors = state.structures.filter(x =>
    !x.closed && x.id !== s.id && (x.qualita||0) >= segMinQ && (x.qualita||0) <= segMaxQ
  ).length;

  const compMul = 1 / (1 + 0.03 * competitors); // -3% per concorrente
...
if (marketing > 0) {
  ...
  const marketingBoost = 1 + (marketing / 5000) * 0.10 * marketingEffect;
  occ *= marketingBoost;
}
...
// === Cap Occupancy e Bonus Recensione (silenzioso) ===
if (occ > 1.0) {
  occ = 1.0;
  s.recensioni = Math.min(s.recensioni + 1.5, 10);
} else if (occ >= 0.995 && occ <= 1.0) {
  // tolleranza: tra 99.5% e 100%
  s.recensioni = Math.min(s.recensioni + 1.5, 10);
}
    s.lastNotti = nights;
...
function finalizeAnnualAndShowReport(){
  ...
  // interessi prestiti 10% e prestito auto se budget < -50k
  state.players.forEach(p=>{
    ensureYearBook(p.id);
    let interest=0;
    (p.loans||[]).forEach(L=> interest += Math.round((L.principal||L)*0.10));
    if(interest>0){ p.budget -= interest; state.annualBook[p.id].interest += interest; log(`Interessi prestiti ${p.name}: €${fmt(interest)}`); }

    if(p.budget < -50000){
      const need = Math.abs(p.budget);
      if(!p.loans) p.loans=[];
      p.loans.push({principal:need, rate:0.10, year:prevYear});
      p.budget += need;
      log(`${p.name} prestito automatico €${fmt(need)} (10% annuo)`);
    }
  });

  openAnnualReport(true);

if (state.access?.isMaster && window.saveToFirebase) window.saveToFirebase(state);
...
function openAnnualReport(fromFinalize){
  const y = fromFinalize ? state.year-1 : state.year;
  const cont = document.getElementById('annualReportContent');
  cont.innerHTML='';

  state.players.forEach(p=>{
    ensureYearBook(p.id);
    const b=state.annualBook[p.id];
    ...
    box.innerHTML=`
      ...
      <table>
        ...
        <tr>
          ...
          <td><strong>€ ${fmt(net)}</strong></td><td><strong>€ ${fmt(p.budget)}</strong></td>
        </tr>
      </table>
    `;
    cont.appendChild(box);

    // push storico annuale
    p.historyAnnual.push({ year:y, gross, mgmt, taxes, upgrades:up, interest:intr, marketing:mkt, extra, net, budget:p.budget });
  });

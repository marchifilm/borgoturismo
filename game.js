...
window.startFirebaseLive = function startFirebaseLive(){
  ...
  gameRef.onSnapshot((snap) => {
    ...
    // 🔔 AUTO-OPEN REPORT ANNUALE PER TUTTI (MASTER + PLAYER)
    if (state.lastAnnualReport?.ts) {
      if (_lastAnnualShownTs !== state.lastAnnualReport.ts) {
        _lastAnnualShownTs = state.lastAnnualReport.ts;

...
warShock: { active:false, monthsLeft:0, cooldown:0 },
...
function updateWarShock(){
  if (!state.warShock) state.warShock = { active:false, monthsLeft:0, cooldown:0 };

  // se in cooldown, scala e stop
  if (state.warShock.cooldown > 0) {
    state.warShock.cooldown--;
    return;
  }
  ...
  const WAR_PROB = 0.01; // 1% al mese (regola qui)
  if (Math.random() < WAR_PROB){
...
function tickWarShockEndOfMonth(){
  ...
  if (state.warShock.monthsLeft <= 0){
    ...
    state.warShock.cooldown = 24; // 24 mesi senza guerra (2 anni) — regola qui
...
function monthlyShock(){
  if (Math.random() * 100 < state.params.eventProbMonth){
    ...
    if (t === 'borgo') {
      ...
      const borgoScelto = borghi[Math.floor(Math.random() * borghi.length)] || 'Borgo';
...
function updateWarShock(){
  ...
  if (!state.warShock.active && Math.random() < 0.01) {
    state.warShock.active = true;
    state.warShock.monthsLeft = 12;
    log('🌍 Guerra internazionale: turismo premium -70% per 12 mesi');
  }

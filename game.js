...
// ✅ SHIM: se per qualunque motivo premiumMarketFill non è visibile, lo definiamo globalmente
globalThis.premiumMarketFill = globalThis.premiumMarketFill || function premiumMarketFill(seas){
  const demand = (seas && typeof seas.demand === 'number') ? seas.demand : 1;
  const base = 0.70;
  const seasonal = 0.45 * demand;
  // clamp01 potrebbe non essere ancora in scope se hai pasticciato con le graffe:
  const v = base + seasonal;

...
function premiumMarketFill(seas){
  // Quanta domanda totale “premium” c’è nel mese (0..1)
  const base = 0.55;                 // domanda base annuale
  const seasonal = 0.35 * seas.demand; // risposta alla stagionalità
  return clamp01(base + seasonal);   // clamp01 taglia tra 0 e 1
}

function premiumChoiceShare(structs, s, state){
  const BETA_PRICE = 0.0030;
...
// ---------- Q3–Q5: PREMIUM LOGIT ----------
  ...
  // ✅ soft-floor realistico: alto solo se il prezzo è “ragionevole”
  // se priceRatio esplode (es. 1500€), il floor collassa verso 0.
  const floorBase =
    (s.qualita === 3) ? 0.22 :
    (s.qualita === 4) ? 0.32 :
    0.38; // Q5

  // gaussiana sul priceRatio (centrata su 1, finestra ~ +/-35%)

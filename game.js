...
const state = {
  ...
  annualBook: {},
warShock: { active:false, monthsLeft:0, startedAt:null },
...
function monthlyShock(){
  if (Math.random()*100 < state.params.eventProbMonth){
    ...
    // ✅ EVENTO BORG O: scegli UN borgo specifico + descrizione chiara
    if (t === 'borgo') {
      const borghi = Object.keys(state.borgoValues || {});
      const borgoScelto = borghi[Math.floor(Math.random() * borghi.length)] || 'Borgo sconosciuto';

      return {
        active: true,
        target: 'borgo',
        borgo: borgoScelto,          // 👈 importante: memorizza quale borgo
        factor,
        desc: `${sign < 0 ? 'Crisi turistica' : 'Boom turistico'} a ${borgoScelto} (${sign < 0 ? '-' : '+'}20%)`
      };
    }

function updateWarShock(){
  // avvio: 5% al mese, SOLO se non è già attivo
  if (!state.warShock) state.warShock = { active:false, monthsLeft:0, startedAt:null };

  if (!state.warShock.active) {
    if (Math.random() < 0.05) {
      state.warShock.active = true;
      state.warShock.monthsLeft = 12;
      state.warShock.startedAt = `Anno ${state.year}, ${MONTH_NAMES[state.month - 1]}`;
      log(`🪖 Shock: Guerre internazionali — durata 12 mesi. Q3–Q5 -70% notti vendute (tutti i borghi).`);
    }
  }

  // fattore da applicare alle strutture premium (Q3+)
  return state.warShock.active ? 0.30 : 1.0;
}

function tickWarShockEndOfMonth(){
  if (!state.warShock?.active) return;

  state.warShock.monthsLeft -= 1;

  if (state.warShock.monthsLeft <= 0) {
    state.warShock.active = false;
    state.warShock.monthsLeft = 0;
    log(`🕊️ Fine shock: Guerre internazionali terminate.`);
  }
}
...
state.players.forEach(p => {
  ...
  p.loans = p.loans.map(L => {
    // se era un numero "vecchio stile", convertilo in oggetto
    if (typeof L === 'number') return { principal: L, rate: 0.20, year: state.year };
    // se è oggetto ma rate mancante o 10%, portalo a 20%
    if (typeof L === 'object') {
...
const warFactor = updateWarShock();
